...
const ProfilingTopPanel = ({
    ...
    const handlePerfEventToggle = (eventName, isChecked) => {
        ...
        const newEvents = isChecked ? [...currentEvents, eventName] : currentEvents.filter((ev) => ev !== eventName);
    ...
    const perfEvents = [
        ...
        { value: 'instructions', label: 'Instructions', tooltip: 'Instructions executed by the CPU' },
        {
            value: 'cache-misses',
            label: 'Cache Misses',
            tooltip: 'Failed cache lookups - indicates memory access issues',
        },
        { value: 'cache-references', label: 'Cache References', tooltip: 'Total cache access attempts' },
        {
            ...
            tooltip: 'Conditional jump instructions executed',
        },
        {
            value: 'branch-misses',
            label: 'Branch Misses',
            tooltip: 'Mispredicted branches - impacts pipeline efficiency',
        },
        {
            value: 'stalled-cycles-frontend',
            label: 'Stalled Cycles (Frontend)',
            tooltip: 'CPU cycles stalled waiting for instructions',
        },
        {
            value: 'stalled-cycles-backend',
            label: 'Stalled Cycles (Backend)',
            tooltip: 'CPU cycles stalled during execution (CPU-dependent)',
        },
    ];
                        ...
                        <Tooltip
                            title={
                                profilingMode === 'continuous'
                                    ? 'Duration is fixed at 60 seconds for Continuous mode'
                                    : 'Profiling duration in seconds (only editable in Ad Hoc mode)'
                            }>
                                                ...
                                                <Tooltip
                                                    ...
                                                    <FormControlLabel
                                                        control={
                                                            <Checkbox
                                                                checked={
                                                                    profilerConfigs.perf?.events?.includes(
                                                                        event.value
                                                                    ) || false
                                                                }
                                                                onChange={(e) =>
                                                                    handlePerfEventToggle(event.value, e.target.checked)
                                                                }
                                                                size='small'
                                                            />
                                                        }
                                ...
                                <FormControlLabel
                                    control={
                                        <Checkbox
                                            ...
                                            onChange={(e) =>
                                                handleAsyncProfilerConfigChange('enabled', e.target.checked)
                                            }
                                            size='small'
                                        />
                                    }
