...
"""
Dynamic Profiling Routes

This module contains all endpoints related to dynamic profiling control,
heartbeat-based command delivery, and profiling status monitoring.

Endpoints:
- POST /profile_request - Create profiling requests (start/stop)
- POST /heartbeat - Receive heartbeat from agents and deliver commands
- POST /command_completion - Report command execution status
- GET /profiling/host_status - Get profiling status of hosts

These endpoints are registered under two prefixes for backward compatibility:
- /metrics/* (legacy - for existing agents)
- /dynamicprofiling/* (new - clean separation of concerns)
"""
...
def profiling_host_status_params(
    ...
    """
    Parse query parameters for profiling host status filtering.
    """
