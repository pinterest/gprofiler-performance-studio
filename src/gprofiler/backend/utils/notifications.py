...
from backend.config import SLACK_BOT_TOKEN
...
class SlackNotifier:
    ...
    def __init__(self, token: Optional[str] = None, default_channel: Optional[str] = "#gprofiler-notifications"):
        ...
        if not bot_token:
            raise ValueError(
                "Slack bot token must be provided either as parameter or via SLACK_BOT_TOKEN environment variable"
            )
    ...
    def send_message(
        self, text: str, channel: Optional[str] = None, thread_ts: Optional[str] = None, **kwargs
    ) -> SlackResponse:
        """
        ...
        try:
            response = self.client.chat_postMessage(channel=target_channel, text=text, thread_ts=thread_ts, **kwargs)
        ...
        **kwargs,
        ...
        try:
            response = self.client.chat_postMessage(
                channel=target_channel, text=text, blocks=blocks, attachments=attachments, thread_ts=thread_ts, **kwargs
            )
    ...
    def send_alert(
        ...
        if additional_fields:
            fields = [{"title": key, "value": value, "short": True} for key, value in additional_fields.items()]
        ...
        return self.send_rich_message(attachments=attachments, text=f"{title}: {message}", channel=channel)
    ...
    def send_performance_alert(
        ...
        return self.send_alert(
            title=title, message=message, severity=severity, channel=channel, additional_fields=additional_fields
        )
    ...
    def update_message(
        ...
        try:
            response = self.client.chat_update(
                ts=ts, channel=channel, text=text, blocks=blocks, attachments=attachments
            )
    ...
    def delete_message(self, ts: str, channel: str) -> SlackResponse:
        """
        Delete a message.
